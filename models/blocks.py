...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Hugues THOMAS - 06/03/2020, modified by Onur Can Bayrak - 08/04/2025
#
...
class KPConv(nn.Module):
    ...
    def __init__(self, kernel_size, p_dim, in_channels, out_channels, KP_extent, radius,
        ...
        self.lca_module = LocalContextAttention(in_channels)
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        neighb_x = gather(x, new_neighb_inds)  # [n_points, n_neighbors, in_fdim]
        
        # Apply Local Context Attention here
        #lca_module = LocalContextAttention(in_dim=self.in_channels).to(neighb_x.device)
        neighb_x = self.lca_module(neighb_x)  # [n_points, in_fdim]
        neighb_x = neighb_x.unsqueeze(1).expand(-1, all_weights.shape[2], -1)
        # print("all_weights shape:", all_weights.shape)     # beklenen: [N, K]
        # print("neighb_x shape:", neighb_x.shape)           # beklenen: [N, F]
        # Apply distance weights (adjusted for new neighb_x shape)
        weighted_features = torch.matmul(all_weights, neighb_x)  # → [n_points, n_kpoints, in_fdim]
...
class LocalContextAttention(nn.Module):
    ...
    def forward(self, neighbor_feats):
        # neighbor_feats: [N, neighbors, D]
        attn_weights = self.attn_mlp(neighbor_feats)  # [N, neighbors, 1]
        attn_weights = F.softmax(attn_weights, dim=1)
        weighted_feats = (neighbor_feats * attn_weights).sum(dim=1)  # [N, D]
